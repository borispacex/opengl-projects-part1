...
public class Objeto {
	...
	class Grupo {
		...
		ArrayList<Integer> triangulos;	/* Arreglo de índice de triangulos */
		...
		try {
			...
			while ((linea = buffer.readLine()) != null){
				...
				if (linea.length() > 0) {
					if (linea.startsWith("mtllib ")) {			/* nombre del arch. de materiales */
					...
					} else if (linea.startsWith("v ")) {		/* vértice */
						numVertices++;
						...
						if (numTokens > 1) {
							...
							indiceDeGrupo = buscaGrupo(nombre); 	// índice de grupo actual
							if (indiceDeGrupo == -1) {
								grupos.add(new Grupo(nombre));
								indiceDeGrupo = grupos.size() - 1; 	// índice de grupo actual
							}
						}
